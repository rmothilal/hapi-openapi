            ...
            if (operation.security && operation.security.length) {
                for (const secdef of operation.security) {
                    ...
                    for (const securityDefinitionName of securitySchemes) {
                        try {
                            await server.auth.test(securityDefinitionName);
                        } catch (e) {
                            if (e.message === `Unknown authentication strategy: ${securityDefinitionName}`) {
                                server.log(['warn'], `${securityDefinitionName} strategy was not registered.`);
                                continue;
                            }
                        }
                        // The Auth class strategies list is now private and cannot be accessed, the above catch is the only work around at the moment
                        //
